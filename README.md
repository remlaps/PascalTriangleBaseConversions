...
1.  **The Offset Method (Pascal Matrix):** Converts bases by "shifting" the polynomial using a **Pascal (Binomial) Matrix**. This is mathematically equivalent to a Taylor Series expansion.
...
* **Matrix Visualization:** See the exact matrix multiplication steps ($N \times P = R$) used to transform the digits.
* **Batch Processing:** Vectorized implementation converts multiple numbers simultaneously using a single matrix operation.
* **Step-by-Step Normalization:** Visualizes the "Digit Normalization" (carry propagation) process required to clean up the result.
...
4.  **Visualize:** Click **Convert & Visualize**.
    ...
    * It will show the **Transformation Matrix** (Pascal or Diagonal).
...
* **Input Vector ($N$):** The digits of the number.
* **Transformation Matrix ($P$):** An Upper-Triangular Pascal Matrix where entries are $P_{r,c} = \binom{c}{r} \cdot k^{c-r}$.
* **Operation:** $R = N \times P$
...
* **Transformation Matrix ($D$):** A Diagonal Matrix containing powers of the factor $F$.
